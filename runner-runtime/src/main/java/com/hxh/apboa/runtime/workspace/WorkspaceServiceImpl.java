...
public class WorkspaceServiceImpl implements WorkspaceService {
    ...
    public String uploadFile(String sessionId, MultipartFile file) {
        ...
        validateCapacity(TenantUtils.getCurrentTenantCode(), sessionId);
    ...
    public List<WorkspaceFileNode> listFiles(String sessionId) {
        validateSessionId(sessionId);
        Path workspaceDir = Paths.get(SysConst.getWorkspacePath(), sessionId).toAbsolutePath().normalize();
    ...
    private Path getWorkspaceDir(String sessionId) {
        validateSessionId(sessionId);
        Path dir = Paths.get(SysConst.getWorkspacePath(), sessionId).toAbsolutePath().normalize();
    ...
    private void validateCapacity(String tenantCode, String sessionId) {
        String workspacePath = String.format("%s/%s", SysConst.getWorkspacePath(tenantCode), sessionId);
        boolean b = CapacityValidator.validateCapacity(
                workspacePath,
                Double.parseDouble(
        ...
        if (!b) {
            throw new BusinessException("工作空间容量超出限制");
        }
    }

    @Override
    public WorkspaceCapacityVO getCapacity(String sessionId) {
        validateSessionId(sessionId);
        String tenantCode = TenantUtils.getCurrentTenantCode();
        String workspacePath = String.format("%s/%s", SysConst.getWorkspacePath(tenantCode), sessionId);

        // 获取最大容量（MB）
        double maxMB = Double.parseDouble(
                System.getenv().getOrDefault(
                        "WORKSPACE_CAPACITY_MB",
                        String.valueOf(SysConst.WORKSPACE_CAPACITY_MB)));
        ...
        // 获取已用空间
        long usedBytes = FolderUtils.getSizeByRelativePath(workspacePath);

        // 计算百分比
        double percent = maxBytes > 0 ? Math.min(100.0, (double) usedBytes / maxBytes * 100) : 0;

        WorkspaceCapacityVO vo = new WorkspaceCapacityVO();
        vo.setUsedBytes(usedBytes);
        vo.setMaxBytes(maxBytes);
        vo.setUsedReadable(formatReadableSize(usedBytes));
        vo.setMaxReadable(formatReadableSize(maxBytes));
