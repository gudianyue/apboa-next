...
public class FolderUtils {
    ...
    /**
     * 根据绝对路径获取目录所占空间大小（字节），兼容 Unix/Windows
     *
     * @param absolutePath 目录的绝对路径
     * @return 目录总大小（字节），路径无效或不存在时返回0
     */
    public static long getSizeByAbsolutePath(String absolutePath) {
        if (FuncUtils.isEmpty(absolutePath)) {
            return 0;
        }
        Path path = Paths.get(absolutePath);
    ...
    /**
     * 根据相对路径获取目录所占空间大小（字节），相对于系统属性 user.dir，兼容 Unix/Windows
     *
     * @param relativePath 目录的相对路径
     * @return 目录总大小（字节），路径无效或不存在时返回0
     */
    public static long getSizeByRelativePath(String relativePath) {
        if (FuncUtils.isEmpty(relativePath)) {
            return 0;
        }
        String basePath = System.getProperty("user.dir");
        Path fullPath = Paths.get(basePath, relativePath);
        return getSize(fullPath.toFile());
    }

    /**
     * 根据相对路径获取目录所占空间大小（字节），相对于指定的基础路径，兼容 Unix/Windows
     *
     * @param basePath     基础路径
     * @param relativePath 目录的相对路径
     * @return 目录总大小（字节），路径无效或不存在时返回0
     */
    public static long getSizeByRelativePath(String basePath, String relativePath) {
        if (FuncUtils.isEmpty(basePath) || FuncUtils.isEmpty(relativePath)) {
            return 0;
        }
        Path fullPath = Paths.get(basePath, relativePath);
        return getSize(fullPath.toFile());
    ...
    /**
     * 根据绝对路径获取格式化的目录大小，兼容 Unix/Windows
     *
     * @param absolutePath 目录的绝对路径
     * @return 格式化后的大小字符串，如 "1.5 GB"、"256.3 MB"
     */
    public static String getReadableSizeByAbsolutePath(String absolutePath) {
        return formatFileSize(getSizeByAbsolutePath(absolutePath));
    }

    /**
     * 根据相对路径获取格式化的目录大小，相对于系统属性 user.dir，兼容 Unix/Windows
     *
     * @param relativePath 目录的相对路径
     * @return 格式化后的大小字符串，如 "1.5 GB"、"256.3 MB"
     */
    public static String getReadableSizeByRelativePath(String relativePath) {
        return formatFileSize(getSizeByRelativePath(relativePath));
    }
