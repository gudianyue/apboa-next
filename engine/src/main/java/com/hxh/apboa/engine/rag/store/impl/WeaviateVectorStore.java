...
public class WeaviateVectorStore implements VectorStore {
    ...
    public void storeEmbedding(Long id, Long chunkId, Long documentId,
        ...
        properties.put("tenant_id", String.valueOf(TenantUtils.getCurrentTenantId()));
    ...
    public void storeEmbeddings(List<EmbeddingRecord> records) {
        ...
        for (EmbeddingRecord record : records) {
            ...
            Map<String, Object> properties = new HashMap<>();
            properties.put("chunk_id", String.valueOf(record.chunkId()));
            properties.put("document_id", String.valueOf(record.documentId()));
            properties.put("knowledge_base_config_id", String.valueOf(record.knowledgeBaseConfigId()));
            properties.put("tenant_id", String.valueOf(tenantId));

            Float[] vector = new Float[dimension];
            for (int i = 0; i < dimension; i++) {
                vector[i] = record.embedding()[i];
            }

            String uuid = UUID.nameUUIDFromBytes(String.valueOf(record.id()).getBytes()).toString();
            Result<io.weaviate.client.v1.data.model.WeaviateObject> result = weaviateClient.data().creator()
                    .withID(uuid)
                    .withClassName(className)
                    .withProperties(properties)
        ...
        WhereFilter whereFilter = WhereFilter.builder()
                ...
                .operands(
                    ...
                    WhereFilter.builder()
                            .path("tenant_id")
                            .operator("Equal")
                            .valueText(String.valueOf(tenantId))
                            .build()
                )
                .build();
