...
public class SecretKeyServiceImpl extends ServiceImpl<SecretKeyMapper, SecretKey> implements SecretKeyService {
    ...
    public SecretKeyVo create(SecretKeyVo vo) {
        ...
        UserDetail userDetail = UserDetail.builder()
                ...
                .name(vo.getName())
                .username(vo.getName())
                .tenantId(TenantUtils.getCurrentTenantId())
                .tenantCode(TenantUtils.getCurrentTenantCode())
                .build();

        // 生成token，id为SK的数据库ID，subject为name，并压缩以缩短存储长度
        String token = TokenUtils.createToken(String.valueOf(entity.getId()), userDetail, ttlMillis);
