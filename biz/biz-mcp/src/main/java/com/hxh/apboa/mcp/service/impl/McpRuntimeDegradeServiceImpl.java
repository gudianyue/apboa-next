...
public class McpRuntimeDegradeServiceImpl implements McpRuntimeDegradeService {
    ...
    private static final DefaultRedisScript<String> RECORD_FAILURE_SCRIPT = buildScript("""
            ...
            seq = seq + 1
            count = count + 1
            redis.call('HSET', key, 'failure_count', tostring(count))
            redis.call('HSET', key, 'event_seq', tostring(seq))
            redis.call('HSET', key, 'last_event_type', 'FAILURE')
    ...
    private static final DefaultRedisScript<String> RECORD_SUCCESS_SCRIPT = buildScript("""
            ...
            redis.call('HSET', key, 'event_seq', tostring(seq))
            redis.call('HSET', key, 'last_event_type', 'SUCCESS')
            redis.call('EXPIRE', key, ttlSeconds)
    ...
    @Override
    public void recordSuccess(Long serverId, Long activationRevision, String configHash, Integer runtimeFailThreshold) {
        recordSuccess(serverId, activationRevision, configHash, runtimeFailThreshold, null);
    }
    ...
    @Override
    public void recordFailure(Long serverId, Long activationRevision, String configHash, Integer runtimeFailThreshold, Throwable throwable) {
        recordFailure(serverId, activationRevision, configHash, runtimeFailThreshold, throwable, null);
    }
    ...
    private String buildKey(Long serverId, Long activationRevision, String configHash) {
        return RedisKeyBuilder.tenantKey("mcp:runtime:degrade:" + serverId + ":" + activationRevision + ":" + configHash);
    }
