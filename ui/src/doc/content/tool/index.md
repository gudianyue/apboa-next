...
:::tip
在线工具目前支持同步工具。异步或流式工具请自行在 `com.hxh.apboa.engine.tool.builtins` 包下编写。
:::
...
- **查看详情**：卡片菜单 → 查看，详情弹窗展示以下字段：

| 字段 | 说明 |
|------|------|
| **关联智能体** | 引用此工具的智能体列表 |
| **工具类型** | 内置（BUILTIN）或自定义（CUSTOM） |
| **分类** | 工具所属分类 |
| **名称** | 工具显示名称 |
| **工具编号** | 工具唯一标识符 |
| **类路径** | 内置工具的 Java 类路径 |
...
| **是否需要确认** | 是 / 否 |
| **语言** | 自定义工具的编程语言 |
| **输入参数** | 参数列表，包含名称、类型、是否必填、默认值 |
| **代码** | 自定义工具的完整实现代码 |

- **编辑工具**：卡片菜单 → 编辑（仅自定义工具）
- **启用/禁用**：卡片菜单 → 启用/禁用，禁用的工具不会参与智能体调用；若工具正在被智能体引用，禁用时会弹出二次确认提示
- **删除工具**：卡片菜单 → 删除（仅自定义工具），若工具被智能体引用会弹出二次确认框

### 搜索与筛选

工具管理支持**双维度筛选**：

- **类型筛选**：页面顶部分段控制器切换全部/内置/自定义，筛选为「内置」时新增卡片自动隐藏
- **分类筛选**：右侧下拉选择器选择分类，分类列表从数据库中所有工具的 `category` 字段去重获取
- **名称搜索**：输入关键词后回车或点击搜索图标搜索

:::tip
类型和分类筛选可以同时使用，例如筛选「自定义」类型下「查询」分类的工具。
:::
...
|------|----------|
| 查看工具列表与详情 | 所有租户成员 |
| 创建、编辑、删除自定义工具 | 租户编辑者、租户管理员、租户所有者 |
| 启用/禁用工具 | 租户编辑者、租户管理员、租户所有者 |
...
- **内置工具**：随系统启动自动注册，通过 Java 类实现，不可编辑和删除，仅可启用/禁用
- **自定义工具**：用户通过在线编写代码创建，支持完整 CRUD
- 筛选为「内置」类型时，新增卡片自动隐藏

### 自定义工具的参数顺序为什么重要？

- 输入参数的顺序必须与 `execute` 方法中接收参数的顺序保持一致
- 参数按照定义的顺序依次传入 `args` 数组
- 例如：第一个定义的参数在 `args[0]`，第二个在 `args[1]`
- 如果顺序不匹配，会导致参数解析错误

### 「是否需要确认」功能怎么不生效？

- 工具开启「需要确认」后，还需要在智能体配置中配合使用：
  1. 进入智能体配置 → 工具与能力 → 钩子配置，选择内置钩子 `IConfirmationHook`
  2. 进入高级设置，启用「启用记忆持久化」
  3. 刷新对话页面
- 以上步骤缺一不可
...
- 工具描述直接影响大模型对工具的理解和选择准确性
- 好的描述应包含：功能说明、参数含义、调用场景、适用范围和限制条件
- 差的描述（如「获取天气」）会导致模型无法准确判断何时调用工具
- 参考文档中的「工具描述编写技巧」章节获取详细指导

### 如何删除被智能体引用的工具？

- 系统会弹出二次确认框，显示引用的智能体名称
- 可以直接确认删除，但引用方的工具集将缺失该工具
- 建议先在相关智能体配置中解除工具关联，再执行删除

### 自定义工具支持异步吗？

- 在线工具目前仅支持同步模式
- 异步或流式工具请在 `com.hxh.apboa.engine.tool.builtins` 包下编写 Java 代码
- 编写完成后重启服务即可注册为内置工具

### 输入参数的 JSON Schema 怎么写？

- 每个参数需要定义：名称（name）、类型（type）、描述（description）、是否必填（required）
- 支持的参数类型：string、integer、number、boolean、array、object
- 参数类型会根据 Java 方法签名自动推导，也支持手动指定
