 ...
 * Props 定义
 *
 * @param markdownFile - markdown 文件的动态 import（?raw 格式）
 * @param primaryColor - 模块主配色，用于目录选中态高亮
 */
const props = withDefaults(defineProps<{
  markdownFile: () => Promise<{ default: string }>
  primaryColor?: string
}>(), {
  primaryColor: '#0F74FF'
})
 ...
 * 目录选中态颜色
 *
 * 对于较浅的主色（如绿色、浅蓝），使用更深同色系以确保白底文字对比度
 */
const tocActiveColor = computed(() => {
  const color = props.primaryColor
  // 需要加深处理的浅色系
  const darkenMap: Record<string, string> = {
    '#66BB6A': '#388E3C',  // MCP 绿 → 深绿
    '#42A5F5': '#1565C0',  // 知识库蓝 → 深蓝
  }
  return darkenMap[color.toUpperCase()] ?? darkenMap[color.toLowerCase()] ?? color
})
...
const loadMarkdownContent = async () => {
  loading.value = true
  try {
    const module = await props.markdownFile()
      ...
      <nav class="doc-md-toc-nav">
        <template v-for="h1 in tocItems" :key="h1.id">
          <div
            ...
            :style="activeSection === h1.id ? { color: tocActiveColor, backgroundColor: primaryColor + '14' } : {}"
          ...
          <template v-for="h2 in h1.children" :key="h2.id">
            <div
              ...
              :style="activeSection === h2.id ? { color: tocActiveColor, backgroundColor: primaryColor + '14' } : {}"
            ...
            <template v-for="h3 in h2.children" :key="h3.id">
              <div
                ...
                :style="activeSection === h3.id ? { color: tocActiveColor, backgroundColor: primaryColor + '14' } : {}"
