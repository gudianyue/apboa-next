 ...
 * PUT /skill/{id}/tools
 */
export function saveTools(id: string, toolIds: string[]) {
  return request.put<ApiResponse<boolean>>(`/api/skill/${id}/tools`, toolIds)
}
 ...
 * POST /skill/{skillId}/sync-to-file
 */
export function syncToFile(skillId: string) {
  return request.post<ApiResponse<boolean>>(`/api/skill/${skillId}/sync-to-file`)
}
