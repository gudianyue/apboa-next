...
/**
 * 触发文档文本提取
 * POST /api/runtime/parse-text/{id}
 */
export function parseText(id: string) {
  return request.post<ApiResponse<boolean>>(`/api/runtime/parse-text/${id}`)
}

/**
 * 读取 .runtime 文件的纯文本内容
 * GET /api/runtime/file-text/{id}
 */
export function getFileText(id: string) {
  return request.get<string>(`/api/runtime/file-text/${id}`, {
    responseType: 'text'
  })
}
