...
// 监听当前索引变化，加载媒体（必须在 computed 之后，否则 isText 等处于 TDZ）
...
<template>
  ...
  >
    <div class="media-preview-container">
      <!-- 媒体内容区域（占满全屏，header/thumbnails 浮于其上） -->
      <div class="media-preview-content">
        ...
        <!-- 文档文件引导下载 -->
        <div v-else-if="isText" class="media-preview-unsupported">
          <FileTextOutlined class="media-preview-unsupported-icon" />
          <p>{{ currentItem?.name || '文档文件' }}</p>
