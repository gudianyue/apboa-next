...
watch(() => userInfo.value, (val) => {
  if (val) {
    editForm.value = {
      ...
      email: val.email,
      rememberLastTenant: val.rememberLastTenant
    }
  }
}, { immediate: true })
...
<template>
  <div>
    ...
    >
      ...
      <AFormItem label="记住当前组织" name="rememberLastTenant">
        <ASwitch v-model:checked="editForm.rememberLastTenant" />
        <span class="form-hint ml-sm">开启后，系统将记住本次登录的组织，下次登录时将直接登录此组织</span>
      </AFormItem>
      <AFormItem>
