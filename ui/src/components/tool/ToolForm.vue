...
<template>
  ...
  >
    <AAlert
      ...
      type="info"
      message="在线工具目前支持同步工具。异步或流式工具请自行在后端代码中使用硬编码实现。"
      banner
      closable
    />
      ...
      <template v-if="!isBuiltin">
        <AFormItem label="输入参数">
          <div class="input-schema-list">
            ...
            <div v-if="formData.inputSchema.length > 1" class="text-placeholder">注意：参数名就是方法参数 params 的 key</div>
          </div>
        </AFormItem>

        <AFormItem label="代码" name="code">
          <div class="tool-tip">
            <span>Tool 基于 Groovy 4.0 引擎执行，请注意使用 Groovy 兼容语法，避免 Java 16+ 特性。</span>
          </div>
          <SmartCodeEditor
            v-if="visible"
