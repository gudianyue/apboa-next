...
/**
 * 手动同步技能文件到 runner-file 节点
 */
async function handleSyncToFile() {
  syncing.value = true
  try {
    await skillApi.syncToFile(skillId.value)
    notSyncing.value = false
    message.success('同步请求已发送，运行节点将自动更新技能文件')
  } finally {
    syncing.value = false
  }
}
  ...
  Modal.confirm({
    ...
    async onOk() {
      ...
      if (res.data.code === 200) {
        ...
        await router.replace({ name: 'SkillEditor', params: { id: newId } })
      }
    },
    onCancel() {
      // 用户取消创建，返回列表
