package com.hxh.apboa.node.db.insert;
...
/**
 * 描述：数据库插入节点
 * 根据配置的数据源ID获取数据源连接，执行 SQL 插入操作并返回影响行数。
 *
 * @author huxuehao
 **/
public class DbInsertNode extends EnhancedNode implements DBNode {
    ...
    public DbInsertNode(String id, String name, Config config) {
        super(id, name, NodeType.DB_INSERT);
        this.config = config;
    ...
    private NodeOutput successNodeOutput(Map<String, Object> inputs, NodeOutput output) {
        ...
        int affected = executor.insert(config.getSql(), params);
