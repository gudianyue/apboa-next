...
/**
 * 描述：数据库更新节点
 * 根据配置的数据源ID获取数据源连接，执行 SQL 更新操作并返回影响行数。
 *
 * @author huxuehao
 **/
public class DbUpdateNode extends EnhancedNode implements DBNode {
    ...
    public DbUpdateNode(String id, String name, Config config) {
        super(id, name, NodeType.DB_UPDATE);
        this.config = config;
    ...
    private NodeOutput successNodeOutput(Map<String, Object> inputs, NodeOutput output) {
        ...
        int affected = executor.update(config.getSql(), params);
