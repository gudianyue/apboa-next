package com.hxh.apboa.node.cache.remove;
...
/**
 * 描述：缓存删除节点
 * 根据配置的缓存ID获取 Redis 连接，从 Redis 中删除指定 key。
 *
 * @author huxuehao
 **/
public class CacheRemoveNode extends EnhancedNode {

    @Getter
    private final Config config;
    private final TemplateFormatter formatter;

    public CacheRemoveNode(String id, String name, Config config) {
        super(id, name, NodeType.CACHE_REMOVE);
        this.config = config;
